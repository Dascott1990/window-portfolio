...
const AI_SCENES = ["Portrait","Landscape","Food","Night Scene","Architecture","Macro","Motion","Text","Low Light","Backlit"];

const FILTERS = {
  photo: "", portrait: "saturate(1.15) contrast(1.05)",
...
const Camera = ({ onClose }) => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (st.mode === "ai" && st.faceCount > 0) {
        ...
        ctx.font = "11px 'SF Mono', monospace"; ctx.fillStyle = "#00f5d4";
        for (let i = 0; i < st.faceCount; i++) {
          const bx = 80 + i * 140, by = 100 + (i % 2) * 60, bw = 110, bh = 130, cs = 18;
          [[bx,by],[bx+bw,by],[bx,by+bh],[bx+bw,by+bh]].forEach(([cx,cy],qi) => {
            ctx.beginPath();
            ctx.moveTo(cx + (qi % 2 === 0 ? cs : -cs), cy); ctx.lineTo(cx, cy);
            ctx.lineTo(cx, cy + (qi < 2 ? cs : -cs)); ctx.stroke();
          });
  ...
  const capturePhoto = useCallback((fromBurst = false) => {
    ...
    if (st.mode === "pro") {
      ctx.font = "14px 'SF Mono', monospace"; ctx.fillStyle = "rgba(255,255,255,0.65)";
  ...
  const captureBurst = useCallback(async () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      capturePhoto(true); set({ burstCount: i + 1 });
    }
    note("✓ Burst complete — 10 frames"); set({ burstCount: 0 });
  ...
  const startTimerCapture = useCallback(() => {
    ...
    timerRef.current = setInterval(() => {
      count--;
      if (count <= 0) { clearInterval(timerRef.current); set({ timerCountdown: null }); capturePhoto(); }
      else set({ timerCountdown: count });
    }, 1000);
  ...
  const onTouchMove = (e) => {
    if (e.touches.length === 2 && touchStartRef.current) {
      const dx = e.touches[0].clientX - e.touches[1].clientX;
      const dy = e.touches[0].clientY - e.touches[1].clientY;
      const newZ = clamp(touchStartRef.current.zoom * (Math.hypot(dx, dy) / touchStartRef.current.dist), 1, 5);
      set({ zoom: parseFloat(newZ.toFixed(2)) }); zoomMV.set(newZ);
    }
  };
  ...
  const PRO_CONTROLS = [
    ...
    { label: "CT",  key: "contrast",   min: -50, max: 50, step: 1 },
    { label: "SAT", key: "saturation", min: -50, max: 50, step: 1 },
  ];
    ...
    >
      ...
      >
        ...
        <AnimatePresence>
          {overlay.bars && (<>
            <motion.div initial={{ scaleY: 0 }} animate={{ scaleY: 1 }} exit={{ scaleY: 0 }}
              style={{ position:"absolute",top:0,left:0,right:0,height:"8.5%",background:"#000",zIndex:10,originY:0 }}
              transition={{ duration: 0.4 }} />
            <motion.div initial={{ scaleY: 0 }} animate={{ scaleY: 1 }} exit={{ scaleY: 0 }}
              style={{ position:"absolute",bottom:0,left:0,right:0,height:"8.5%",background:"#000",zIndex:10 }}
              transition={{ duration: 0.4 }} />
          </>)}
        </AnimatePresence>
        ...
        <AnimatePresence>
          {focusRing && (
            <motion.div key={focusRing.key}
              initial={{ scale:1.8, opacity:1 }} animate={{ scale:1, opacity:1 }} exit={{ scale:0.9, opacity:0 }}
              transition={{ duration:0.35, ease:"easeOut" }}
              style={{ position:"absolute", left:focusRing.x-28, top:focusRing.y-28, width:56, height:56,
                border:`1.5px solid ${modeColor}`, borderRadius:4, zIndex:20 }} />
          )}
        </AnimatePresence>
        ...
        <AnimatePresence>
          {st.timerCountdown !== null && (
            <motion.div key={st.timerCountdown}
              ...
              style={{ zIndex:30 }}>
              <span style={{ fontSize:96, fontWeight:700, color:modeColor,
                textShadow:`0 0 40px ${modeColor}88, 0 0 80px ${modeColor}44` }}>{st.timerCountdown}</span>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <AnimatePresence>
          {st.burstCount > 0 && (
            <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0 }}
              className="absolute bottom-1/2 left-1/2 -translate-x-1/2 pointer-events-none" style={{ zIndex:30 }}>
        ...
        <AnimatePresence>
          {st.notification && (
            <motion.div
              initial={{ y:-60, scale:0.85, opacity:0 }} animate={{ y:0, scale:1, opacity:1 }}
              exit={{ y:-40, scale:0.9, opacity:0 }}
              transition={{ type:"spring", damping:20, stiffness:300 }}
              style={{ position:"absolute", top:16, left:"50%", transform:"translateX(-50%)",
                background:"rgba(20,20,28,0.92)", backdropFilter:"blur(20px)",
                border:`1px solid ${modeColor}44`, borderRadius:20, padding:"8px 18px",
                color:"#fff", fontSize:13, fontWeight:500, zIndex:50, whiteSpace:"nowrap",
                boxShadow:`0 8px 32px rgba(0,0,0,0.6), 0 0 0 1px ${modeColor}22` }}>
              <span style={{ color:modeColor, marginRight:6 }}>●</span>{st.notification}
            </motion.div>
          )}
        </AnimatePresence>
          ...
          {st.isRecording && (
            <motion.div initial={{ opacity:0, scale:0.8 }} animate={{ opacity:1, scale:1 }} exit={{ opacity:0 }}
              style={{ position:"absolute", top:70, left:"50%", transform:"translateX(-50%)",
                ...
                display:"flex", alignItems:"center", gap:8, padding:"6px 14px", zIndex:20 }}>
              ...
              <span style={{ color:"#fff", fontSize:13, fontWeight:600 }}>{fmtTime(st.recordSeconds)}</span>
            </motion.div>
          )}
        </AnimatePresence>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={toggleFlashlight}
                    style={{ width:36, height:36, borderRadius:"50%",
                      background: st.flashMode !== "off" ? modeColor+"44" : "rgba(255,255,255,0.12)",
                      border:"none", cursor:"pointer", display:"flex", alignItems:"center",
                      justifyContent:"center", color: st.flashMode !== "off" ? modeColor : "#fff" }}>
                    <Icon d={st.flashMode !== "off" ? ICONS.flash : ICONS.flashOff} size={16} />
                  </button>
                  <button onClick={() => { tog("gridEnabled"); showControls(); }}
                    style={{ width:36, height:36, borderRadius:"50%",
                      background: st.gridEnabled ? modeColor+"44" : "rgba(255,255,255,0.12)",
                      border:"none", cursor:"pointer", display:"flex", alignItems:"center",
                      justifyContent:"center", color: st.gridEnabled ? modeColor : "#fff" }}>
                ...
                {st.aiScene && (
                  <motion.div key={st.aiScene}
                    ...
                    style={{ background:"rgba(0,0,0,0.55)", backdropFilter:"blur(12px)",
                      borderRadius:12, padding:"4px 12px", border:`1px solid ${modeColor}55`,
                      fontSize:11, fontWeight:600, color:modeColor, letterSpacing:"0.06em" }}>
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => { tog("hdr"); showControls(); }}
                    style={{ padding:"4px 10px", borderRadius:8,
                      background: st.hdr ? modeColor+"44" : "rgba(255,255,255,0.12)",
                      border:"none", cursor:"pointer",
                      color: st.hdr ? modeColor : "rgba(255,255,255,0.7)",
                      fontSize:11, fontWeight:700, letterSpacing:"0.05em" }}>
                    HDR
                  </button>
                  <button onClick={() => { tog("scanMode"); showControls(); }}
                    style={{ width:36, height:36, borderRadius:"50%",
                      background: st.scanMode ? "#00f5d433" : "rgba(255,255,255,0.12)",
                      border:"none", cursor:"pointer", display:"flex", alignItems:"center",
                      justifyContent:"center", color: st.scanMode ? "#00f5d4" : "#fff" }}>
                    <Icon d={ICONS.scan} size={16} />
                  </button>
                  <div style={{ padding:"3px 8px", borderRadius:8, background:"rgba(0,0,0,0.55)",
                    fontSize:10, fontWeight:700,
                    color: st.fps >= 50 ? "#4ade80" : "#facc15", letterSpacing:"0.06em" }}>
                    {st.fps}fps
                  </div>
                </div>
              </div>
        ...
        {st.controlsVisible && (
          ...
          >
              ...
              {st.mode === "pro" && (
                <motion.div initial={{ height:0, opacity:0 }} animate={{ height:"auto", opacity:1 }}
                  ...
                  <div className="flex items-center gap-4 px-4 py-2 overflow-x-auto" style={{ scrollbarWidth:"none" }}>
                    ...
                    <div style={{ flexShrink:0, textAlign:"center", minWidth:80 }}>
                      <div style={{ color:"#facc15", fontSize:9, fontWeight:700, letterSpacing:"0.1em", marginBottom:4 }}>ISO</div>
                      <select value={st.iso} onChange={(e) => set({ iso: e.target.value })}
                        style={{ background:"rgba(255,255,255,0.1)", border:"1px solid rgba(255,255,255,0.15)",
                          color:"#fff", borderRadius:6, padding:"2px 6px", fontSize:11, cursor:"pointer" }}>
                    ...
                    <div style={{ flexShrink:0, textAlign:"center", minWidth:80 }}>
                      <div style={{ color:"#60a5fa", fontSize:9, fontWeight:700, letterSpacing:"0.1em", marginBottom:4 }}>WB</div>
              ...
              <div className="flex items-center justify-center gap-1 overflow-x-auto px-4"
                ...
                {MODES.map((m) => (
                  <motion.button key={m.id}
                    onClick={() => { set({ mode: m.id }); note(`Mode: ${m.label}`); showControls(); }}
                    whileTap={{ scale:0.94 }}
                    ...
                    <span style={{ fontSize:11, fontWeight: st.mode === m.id ? 700 : 400,
            ...
            <div className="flex items-center justify-center gap-3 py-2 px-4">
              {[1, 1.5, 2, 3, 5].map((z) => (
                <motion.button key={z}
                  onClick={() => { set({ zoom: z }); zoomMV.set(z); showControls(); }}
                  whileTap={{ scale:0.9 }}
                  style={{ width: st.zoom === z ? 44 : 34, height: st.zoom === z ? 44 : 34,
                    borderRadius:"50%",
                    ...
                    fontSize: st.zoom === z ? 12 : 10, fontWeight:700,
            ...
            <div className="flex items-center justify-between px-8 pb-4 pt-1" style={{ minHeight:80 }}>
              ...
              <div className="flex flex-col items-center gap-2">
                <motion.button whileTap={{ scale:0.9 }}
                  onClick={() => {
                    ...
                    set({ timer: next }); note(next === 0 ? "Timer off" : `Timer: ${next}s`); showControls();
                  }}
                ...
                <motion.button whileTap={{ scale:0.9 }}
                  ...
                  style={{ width:44, height:44, borderRadius:"50%",
                    background: st.capturedImages.length ? "rgba(255,255,255,0.1)" : "rgba(255,255,255,0.04)",
              ...
              <motion.button onClick={handleShutter} whileTap={{ scale:0.92 }}
                style={{ width:72, height:72, borderRadius:"50%", cursor:"pointer",
                  ...
                  padding:4, background:"transparent", flexShrink:0 }}>
                <motion.div
                  animate={{ scale: st.mode === "burst" ? [1, 1.08, 1] : 1,
                    borderRadius: st.mode === "video" && st.isRecording ? "8px" : "50%",
                    background: st.mode === "video" ? (st.isRecording ? "#ef4444" : "#ef4444cc") : modeColor }}
        ...
        {st.previewImage && (
          ...
          >
            ...
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between",
              ...
              onClick={(e) => e.stopPropagation()}>
              <button onClick={() => set({ previewImage:null, previewIndex:null })}
                ...
                {st.previewIndex > 0 && (
                  <button onClick={(e) => { e.stopPropagation(); nextPreview(); }}
                    style={{ position:"absolute", right:12, top:"50%", transform:"translateY(-50%)",
                      ...
                      border:"1px solid rgba(255,255,255,0.2)", cursor:"pointer",
                      display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", zIndex:10 }}>
                    <Icon d={ICONS.chevRight} size={18} />
                  </button>
                )}
              </>
            )}
      ...
      <AnimatePresence>
        {st.showGallery && (
          <motion.div
            initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
            style={{ position:"fixed", inset:0, bottom:"var(--taskbar-height,52px)",
              zIndex:75, background:"rgba(0,0,0,0.92)", backdropFilter:"blur(24px)" }}
      ...
      <AnimatePresence>
        {st.showDevPanel && (
          <motion.div
            ...
            onClick={(e) => e.stopPropagation()}
            style={{ position:"fixed", top:0, right:0, bottom:"var(--taskbar-height,52px)", width:260, zIndex:60,
              background:"rgba(8,8,14,0.97)", backdropFilter:"blur(28px)",
            ...
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
              ...
              <button onClick={() => set({ showDevPanel:false })}
